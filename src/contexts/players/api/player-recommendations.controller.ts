...
@Controller('players')
export class PlayerRecommendationsController {
  constructor(
  ...
  createRecommendation(@Req() req: RequestWithUser, @Param('slug') slug: string, @Body() body: CreatePlayerRecommendationDTO) {
    const authorProfileId = req.user?.pid ?? '';
    const authorSlug = req.user?.slug ?? '';
    return this.createRecommendationUseCase.execute(authorProfileId, authorSlug, 'player', slug, body);
  }
