...
import {
  AUTH_REPOSITORY,
  AuthRepositoryPort,
} from '@/contexts/auth/app/ports/auth.repository.port';
import { AuthEmailAlreadyUsedError } from '@/contexts/auth/domain/errors/auth.errors';
import {
  PlayerInvalidLanguagesError,
  PlayerInvalidNationalityError,
  PlayerNotFoundError,
} from '../../domain/errors/player-profile.errors';
import {
  PLAYER_REPOSITORY,
  PlayerCredentialsUpdateInput,
...
export class UpdatePlayerProfileUseCase {
  ...
  async execute(
    slug: string,
    dto: UpdatePlayerProfileRequestDto,
    isAdmin: boolean,
  ): Promise<PlayerPrivateProfileResponse> {
    ...
    if (dto.description !== undefined)
      profileUpdates.description = dto.description;
    ...
    if (dto.bannerPicture !== undefined)
      profileUpdates.bannerPicture = dto.bannerPicture;
    if (dto.phone !== undefined) profileUpdates.phone = dto.phone;
    if (dto.workSector !== undefined)
      profileUpdates.workSector = dto.workSector;
    if (dto.contractType !== undefined)
      profileUpdates.contractType = dto.contractType;
    if (dto.isDisabledPlayer !== undefined)
      profileUpdates.isDisabledPlayer = dto.isDisabledPlayer;
    ...
    if (dto.nationalityId !== undefined || dto.languageIds !== undefined) {
      ...
      if (dto.languageIds !== undefined && dto.languageIds !== null) {
        ...
        const invalidIds = dto.languageIds.filter(
    ...
    if (dto.nationalityId !== undefined)
      profileUpdates.nationalityId = dto.nationalityId;
    if (dto.languageIds !== undefined)
      profileUpdates.languageIds = dto.languageIds;
    ...
    await this.repo.updateProfile(context, {
      ...
      credentials: Object.keys(credentialsUpdates).length
        ? credentialsUpdates
        : undefined,
    });
    ...
    const scores = await this.playerProfileScoreService.computeForProfile(
      updated.profile,
    );
