...
export class UserLoginUsecase {
  ...
  async execute(dto: UserLoginDto): Promise<UserJwtTokensPresenter> {
    ...
    const isPasswordValid = await this.hasher.compare(dto.password, credentials.passwordHash);
    if (!isPasswordValid) {
      throw new AuthInvalidCredentialsError();
    }

    const profile = await this.authRepo.findProfileByCredentialId(credentials.id);
    if (!profile) {
      throw new AuthInvalidCredentialsError();
    }

    const access_token = await this.tokens.createAccessToken({ sub: credentials.id, pid: profile.id, email: credentials.email, username: profile.username, slug: profile.slug });
    const refresh_token = await this.tokens.createRefreshToken({ sub: credentials.id });

    await this.authRepo.updateCredentialsLastLoginAt(credentials.id, new Date());
